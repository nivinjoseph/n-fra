...
export class WindowsBastionProvisioner
{
    ...
    private readonly _config: WindowsBastionConfig;
    private readonly _instanceType: aws.ec2.InstanceType;


    public constructor(name: string, config: WindowsBastionConfig)
    {
        ...
        given(config, "config").ensureHasValue().ensureIsObject()
            ...
            .ensure(t => t.volumeSize > 0 && t.volumeSize <= 1000,
                "volumeSize must be between 1 and 1000 inclusive");
        this._config = config;
    ...
    public async provision(): Promise<WindowsBastionDetails>
    {
    ...
    private _createIamInstanceProfile(): aws.iam.InstanceProfile | undefined
    {
        const instancePolicies = new Array<PolicyDocument | string>();
        if (this._config.policies != null && this._config.policies.isNotEmpty)
            instancePolicies.push(...this._config.policies);

        const managedPolicies = [
            ...instancePolicies.where(t => typeof t === "string") as Array<string>,
