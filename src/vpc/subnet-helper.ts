...
{
    public static convertBinaryToDecimal(input: string): number
    {
        const byte: Byte = input.trim().split("")
            .map(t => Number.parseInt(t)) as Byte;
        
        return byte.reduce<number>((acc, bit, index) =>
        {
    ...
    public static convertDecimalToBinary(input: number | string): string
    {
    ...
    public static calculateSubnets(cidrRange: string, numSubnets: number): Array<string>
    {
        ...
        const networks = new Array<string>();
        for (let i = 0; i < slot[1]; i++)
        {
            ...
            const decimals = new Array<number>();
            for(let j = 0; j < 4; j++)
            {
    ...
    public static validateCidrRange(cidrRange: string): boolean
    {
        try 
        {
            given(cidrRange, "cidrRange").ensureHasValue().ensureIsString()
