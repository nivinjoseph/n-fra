...
export class ValkeyProvisioner {
    ...
    constructor(name, config) {
        ...
        if (haConfig != null) {
            ...
            given(haConfig.durability, "config.haConfig.durability").ensureIsEnum(ValkeyDurability);
            ...
            given(config, "config").ensure(t => t.nodeType.split(".").length === 3
                && ValkeyProvisioner._durableNodeTypeFamilies.contains(t.nodeType.split(".")[1]), `haConfig requires a node type from one of the following families: ${ValkeyProvisioner._durableNodeTypeFamilies.join(", ")}`);
        }
        this._config = config;
    }
    provision() {
        ...
        const topologyArgs = haConfig != null
            ? {
